...
async function handleCallback() {
  ...
  if (errorParam) {
    error.value = errorDescription || t(`oauth.errors.${errorParam}`, errorParam)
  ...
  try {
    ...
    const response = await authService.exchangeOAuthCode({
      ...
      redirect_uri: storedRedirectUri || window.location.origin + '/oauth/callback'
    })
    ...
    const redirectTo = sessionStorage.getItem('oauth_redirect_to') || '/profile'
