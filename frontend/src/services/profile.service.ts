...
export const profileService = {
  ...
  async getResolved(userId: string): Promise<ResolvedBaseProfile> {
    const response = await api.get<ResolvedBaseProfile>(
  ...
  async addName(
    userId: string,
    data: IdentityNameCreate,
  ): Promise<IdentityName> {
    const response = await api.post<IdentityName>(
      `/profiles/${userId}/names`,
      data,
    );
    return response.data;
  },
  ...
  async updateName(
    ...
    const response = await api.patch<IdentityName>(
      `/profiles/${userId}/names/${nameId}`,
      data,
    );
    return response.data;
  },
