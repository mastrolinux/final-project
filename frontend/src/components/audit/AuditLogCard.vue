...
const hasChanges = computed(() => {
  return props.entry.changes !== null && Object.keys(props.entry.changes).length > 0
})
</script>
    ...
    <div class="audit-card">
      <div class="audit-header">
        <div class="audit-event">
          ...
          <span class="event-time">{{ formatTimestamp(entry.created_at) }}</span>
        </div>
      ...
      <div class="audit-meta">
        <div class="meta-item">
          ...
          <span class="meta-value">
            {{ entry.resource_type }}
            <code class="resource-id">{{ entry.resource_id.substring(0, 8) }}...</code>
          </span>
        </div>
        ...
        <div v-if="entry.actor_id && entry.actor_id !== entry.user_id" class="meta-item">
          <span class="meta-label">Actor:</span>
          <span class="meta-value">
            <code class="resource-id">{{ entry.actor_id.substring(0, 8) }}...</code>
          </span>
        </div>
      </div>
      ...
      <div v-if="hasChanges" class="audit-changes">
        <details>
          ...
          <pre class="changes-content">{{ JSON.stringify(entry.changes, null, 2) }}</pre>
        </details>
      </div>
    </div>
  </BaseCard>
