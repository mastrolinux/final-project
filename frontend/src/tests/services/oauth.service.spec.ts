...
describe("oauthService", () => {
      ...
      const mockResponse: ConsentDetailsResponse = {
        ...
        scopes: [
          ...
          },
          {
            ...
            description: "Name and account type",
            is_sensitive: false,
          },
        ],
        request: {
  ...
  describe("submitConsent", () => {
    it("should submit allow decision and return redirect URL", async () => {
      const mockResponse = {
        redirect_to: "https://example.com/callback?code=xyz&state=abc123",
      };
      ...
      expect(result.redirect_to).toBe(
        "https://example.com/callback?code=xyz&state=abc123",
      );
    });
