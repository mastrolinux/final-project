---
title: "CoBI: Context-Based Identity Management API"
...
The scope includes multi-context identity profile management with an inheritance engine, an OAuth 2.1 authorization server with mandatory PKCE, GDPR-inspired privacy features including data export and deletion with 30-day grace periods, multilingual name support accommodating global conventions via JSONB storage, comprehensive audit logging for transparency and accountability, profile picture management with context-specific overrides, identity document upload for account verification enabling access to legal and healthcare contexts, and a CI/CD pipeline with static analysis enforcing code quality and security standards. From this scope, seven project objectives follow:

1. Multi-context identity profile management with an inheritance engine, six context types, and context-specific overrides including avatar presentation.
2. An OAuth 2.1 authorization server with mandatory PKCE, context-aware token binding, and consent management with withdrawal.
3. Multilingual name support via culturally-neutral JSONB storage without structural naming assumptions.
4. GDPR-inspired privacy features including data export (Article 15) and soft deletion with 30-day retention (Article 17).
5. Audit logging with SHA-256 hash chain integrity verification for accountability and transparency.
6. Identity document upload with encrypted storage and administrative verification workflow.
7. A CI/CD pipeline enforcing lint, type checking, security analysis, and automated testing.
...
The frontend uses Vue 3 with TypeScript, Vite for build tooling, and Pinia for state management. Vue was selected over React and Angular on three criteria: its Composition API provides TypeScript inference comparable to React hooks without requiring JSX tooling, its single-file component format co-locates template, logic, and styles in a way that maps directly to the component-based architecture, and its smaller bundle size (vue@3.5 is approximately 33 KB gzipped versus react@19+react-dom at approximately 44 KB) reduces initial load time on the Render.com free-tier infrastructure where bandwidth is constrained. Pinia replaces Vuex as the recommended Vue state management library, offering type-safe stores without the boilerplate of mutations and actions that Vuex required.
...
The project is named CoBI, an acronym for **Co**ntext-**B**ased **I**dentity. The name reflects the system's design principle: identity is not a fixed, singular record but a context-dependent construct that users control and present differently depending on the situation.

@fig-context illustrates the system context, showing primary actors and their relationships to the CoBI API. Users manage their own profiles and third-party applications request identity data via OAuth. Social authentication providers serve as optional identity verification sources during account creation or login, but the API issues all session tokens and remains the sole authentication authority.
...
flowchart TB
    ...
    api[["CoBI API<br>(Multi-context identity with OAuth 2.1)"]]

    subgraph external["External Systems"]
        thirdparty["Third-Party Apps<br>(OAuth clients)"]
        social["Social Auth Provider<br>(Google)"]
    end

    user -->|"Manages profiles"| api
    thirdparty -->|"Requests identity via OAuth"| api
    api -->|"Verifies user identity at login"| social
```
...
The implementation is organized into nine phases: authentication and security (Phase 1), context profiles with inheritance (Phase 2), OAuth 2.1 authorization server (Phase 3), privacy features (Phase 4), frontend application (Phase 5), deployment (Phase 6), testing (Phase 7), refinement (Phase 8), and final report (Phase 9). Phases 1 through 5 are complete, with Phases 6 through 9 in progress at time of submission. Frontend development began before Phase 4 completion because the consent flow UI was required to validate OAuth integration end-to-end. Phase 4 features, including audit logging, profile picture upload, identity document verification with admin review, and automated document expiry, were completed concurrently with frontend work. Linear tracks 83 tasks across twelve milestones, with 61 completed as of March 2026.
...
The application includes views covering authentication flows (login, registration, password reset, email verification), profile management (view, edit, avatar upload), context management (list, create, detail with verification badges), OAuth consent flow, identity document upload with drag-and-drop, and administrative interfaces for OAuth client management, verification review, and soft-deleted account purging. A design system provides consistent styling with theme support (light/dark mode), accessibility compliance targeting WCAG 2.1 AA, responsive layouts, and internationalization support for English and Italian locales. The application logo was generated using Google Gemini with the Imagen 3 model [@google2025gemini_imagen]. @fig-context-list shows the context profiles list view, where users manage multiple identity presentations, and @fig-context-detail shows the detail view for an individual context with its field overrides.
...
The evaluation assesses functional correctness, performance characteristics, code quality, and API contract adherence following industry standards documented in REST API testing literature and is inspired by the OWASP API Security guidelines.
...
The health endpoint establishes a network baseline with no database query. Its 70ms local-to-production delta represents the round-trip to Render Frankfurt. Subtracting this constant isolates server-side overhead: context resolution takes approximately 183ms on production versus 27ms locally (6.8x), and context listing takes 118ms versus 16ms (7.4x). Both Render and Supabase deploy in the Frankfurt region, so this gap reflects Supabase Micro's shared-CPU query execution rather than geographic distance.
...
Bandit, a Python SAST tool, runs in the CI pipeline at medium severity threshold against all source modules, detecting hardcoded credentials, insecure cryptographic usage, SQL injection risks, and unsafe deserialization. True positives are fixed before merge; false positives from test fixtures receive inline suppression with documented justification.

## User Research and Validation

Semi-structured interviews with four individuals who experienced harm from inadequate context separation validate the project's core motivation. Participants were recruited through online communities focused on digital privacy and identity protection. Each interview lasted approximately 25 minutes, exploring experiences with context collapse, coping strategies, and reactions to the proposed design.
...
A formal usability evaluation complemented the interview findings using the System Usability Scale (SUS), a ten-item questionnaire developed by Brooke [@brooke1996sus]. Six participants were recruited through the same channels as the interviews, with no overlap. Participants ranged in age from 25 to 54 and spanned three technical literacy levels: three beginners, two intermediate, and one advanced user. Prior identity tool experience varied: three had never used such tools, two occasionally, and one regularly. Appendix D provides full per-participant demographics.

Each session lasted approximately 30 minutes and followed a moderated think-aloud protocol. Participants were asked to verbalize their reasoning while completing five tasks designed to exercise the system's core features:

1. Account registration and email verification
2. Context profile creation with field overrides (display name, email, bio)
3. OAuth consent flow: authorizing a third-party application with scope selection
4. Consent revocation via the Connected Apps settings page
5. Data export of personal information

After completing the tasks, participants filled out the standard SUS questionnaire. Scores were computed per the standard formula [@brooke1996sus] and interpreted using the adjective rating scale established by Bangor et al. [@bangor2009sus].

| Participant | Technical Level | SUS Score | Adjective Rating | Tasks Completed |
|-------------|----------------|-----------|-------------------|-----------------|
| P1 | Beginner | 70.0 | Good | 3 of 5 |
| P2 | Intermediate | 80.0 | Good | 4 of 5 |
| P3 | Beginner | 92.5 | Excellent | 5 of 5 |
| P4 | Advanced | 95.0 | Excellent | 5 of 5 |
| P5 | Beginner | 30.0 | Poor | 3 of 5 |
| P6 | Intermediate | 72.5 | Good | 5 of 5 |

: SUS scores by participant with task completion rates {#tbl-sus-scores}

The mean SUS score was 73.3 (SD = 23.5), placing the system in the "acceptable" range. The high standard deviation reflects a bimodal distribution: five participants scored between 70.0 and 95.0, while P5 (beginner, no prior identity tool experience) scored 30.0. Excluding P5, the remaining five participants achieved a mean of 82.0 (SD = 11.4), corresponding to a "good" rating. P5 required facilitator hints to locate the consent revocation interface and could not independently complete the revocation task, indicating that the multi-context model requires onboarding support for non-technical users.

Think-aloud observations identified five improvement areas. Three were resolved before final submission: dark mode contrast on the OAuth consent screen, incomplete Italian localization (44 untranslated keys), and consent revocation discoverability. Two issues remain open: email verification status visibility during registration and OAuth scope terminology (participants found `profile:read:basic` unintuitive compared to plain language descriptions). Full per-participant task metrics and improvement details appear in Appendix D.
...
Seven objectives were defined in @sec-introduction. Evaluation evidence supports achievement of all seven. The SUS evaluation (@tbl-sus-scores) yielded a mean score of 73.3 (SD = 23.5), placing the system in the "acceptable" range per the Bangor et al. adjective scale [@bangor2009sus]. Three of five improvement tasks from usability testing were resolved before submission, with the most significant being a Settings navigation restructure from a single link into a dropdown grouping General settings, Connected Apps, and Data Export.
...
The architecture validates Goffman's theoretical framework [@goffman1959presentation] in a computational context: the inheritance engine enables distinct "front stage" presentations for different audiences derived from a single "back stage" base profile, designing around how humans perform identity rather than forcing adaptation to technical constraints.
...
Usability evaluation through SUS testing with six participants yielded a mean score of 73.3, placing the system in the "acceptable" range. However, high variance (SD = 23.5) and the small sample size prevent generalizable conclusions. The non-technical participant's score of 30 suggests that the multi-context model introduces cognitive complexity requiring onboarding support for users unfamiliar with identity management concepts. Whether the system achieves adequate usability across broader populations and cultural contexts remains an open question requiring larger-scale studies.
...
Cross-context consistency validation could warn users when information shared across contexts with different privacy levels might enable re-identification. 
...
# Appendix F: Access Information {-}

The production deployment is available at [https://identity.okrbusiness.com](https://identity.okrbusiness.com). The complete source code is hosted at [https://github.com/mastrolinux/final-project](https://github.com/mastrolinux/final-project) (access available upon request to lc193@student.london.ac.uk or luca.opensource@gmail.com).

The database is seeded with five test accounts representing the personas described in @sec-design. All accounts share the password `TestPassword123!`.

| Email | Account Type | Role | Context Profiles |
|-------|-------------|------|-----------------|
| sarah.chen@example.com | Verified | Admin | Professional, Legal |
| li.ming@example.com | Unverified | User | Social, Healthcare |
| alex.shadow@example.com | Pseudonymous | User | None |
| sukarno@example.com | Verified | User | None |
| jordan.smith@example.com | Verified | User | Family, Custom |

: Seed accounts for evaluation {#tbl-seed-accounts}

Sarah Chen's admin account provides access to administrative endpoints including OAuth client management, verification document review, and soft-deleted account purging. The pseudonymous account (Alex) demonstrates restricted access: attempts to create legal or healthcare contexts return HTTP 403 as enforced by the ContextService business rules.
