...
class OAuthService:
    ...
    def _assert_context_verified(self, context_profile_id: UUID | None) -> None:
    ...
    def validate_redirect_uri(self, client: OAuthClient, redirect_uri: str) -> None:
        ...
        if not client.is_redirect_uri_valid(redirect_uri):
            raise InvalidRequestError("Invalid redirect_uri: must match one of the registered URIs")
    ...
    def validate_scopes(
        ...
        user_profile: BaseProfile | None = None,
        context_type: ContextType | None = None,
    ) -> list[str]:
        """Validate requested scopes against client permissions, account type,
        ...
        for scope_name in requested_scopes:
            ...
            if not client.can_request_scope(scope_name):
                raise InvalidScopeError(f"Client not authorized for scope: {scope_name}")
            ...
            if scope.required_context_type:
                if not context_type:
                    raise InvalidScopeError(
                        f"Scope '{scope_name}' requires context"
                        f" type '{scope.required_context_type}'"
                    )
    ...
    def get_scope_details(self, scope_names: list[str]) -> list["OAuthScope"]:
        ...
        db_scope_names = [s for s in scope_names if s not in ["openid", "offline_access"]]
    ...
    def filter_profile_fields_by_scopes(
        self, profile_data: dict[str, Any], scopes: list[str]
    ) -> dict[str, Any]:
        ...
        scope_fields = {
            ...
            "contexts:social:read": {"display_name", "bio", "email_override", "interests"},
        }
    ...
    def revoke_token(
        self, token: str, token_type_hint: str | None = None, client_id: str | None = None
    ) -> bool:
    ...
    def check_consent(self, user_id: UUID, client_id: str, required_scopes: list[str]) -> bool:
        """Check if user has granted consent covering all required scopes."""
        return self.oauth_repo.has_valid_consent(
            user_id=user_id, client_id=client_id, required_scopes=required_scopes
        )
