...
class ProfileRepository:
    ...
    def get_profile_by_id(self, user_id: UUID) -> BaseProfile | None:
        """Get profile by user ID, excluding soft-deleted."""
        return (
            self.db.query(BaseProfile)
            .filter(and_(BaseProfile.user_id == user_id, BaseProfile.deleted_at.is_(None)))
            .first()
        )

    def get_profile_by_id_including_deleted(self, user_id: UUID) -> BaseProfile | None:
        """Get profile by user ID, including soft-deleted."""
        return self.db.query(BaseProfile).filter(BaseProfile.user_id == user_id).first()

    def get_profile_by_email(self, email: str) -> BaseProfile | None:
        """Get profile by email address, excluding soft-deleted."""
        return (
            self.db.query(BaseProfile)
            .filter(and_(BaseProfile.primary_email == email, BaseProfile.deleted_at.is_(None)))
            .first()
        )
    ...
    def update_profile(self, user_id: UUID, **updates) -> BaseProfile | None:
    ...
    def soft_delete_profile(self, user_id: UUID) -> bool:
        ...
        profile = self.db.query(BaseProfile).filter(BaseProfile.user_id == user_id).first()
    ...
    def get_identity_name_by_id(self, name_id: UUID) -> IdentityName | None:
        """Get identity name by ID."""
        return self.db.query(IdentityName).filter(IdentityName.id == name_id).first()

    def get_identity_names(
        self, identity_id: UUID, include_deprecated: bool = False
    ) -> list[IdentityName]:
        """Get all identity names for a profile."""
        query = self.db.query(IdentityName).filter(IdentityName.identity_id == identity_id)

        if not include_deprecated:
            query = query.filter(IdentityName.is_deprecated == False)

        return query.order_by(IdentityName.is_primary.desc(), IdentityName.created_at.asc()).all()

    def update_identity_name(self, name_id: UUID, **updates) -> IdentityName | None:
        ...
        name.updated_at = datetime.now(UTC)
    ...
    ) -> tuple[BaseProfile | None, list[IdentityName]]:
