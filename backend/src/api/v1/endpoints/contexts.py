...
def get_context_service(db: Session = Depends(get_db)) -> ContextService:
    ...
    return ContextService(
        context_repo, profile_repo, audit_service=audit_service, auth_repository=auth_repo
    )
...
@router.get("/profiles/{user_id}/contexts", response_model=list[ContextProfileResponse])
def list_user_contexts(
    user_id: UUID,
    include_inactive: bool = Query(
        default=False, description="Include inactive contexts in results"
    ),
    service: ContextService = Depends(get_context_service),
):
...
@router.get("/profiles/{user_id}/contexts/{context_id}", response_model=ContextProfileResponse)
def get_context_profile(
    user_id: UUID, context_id: UUID, service: ContextService = Depends(get_context_service)
):
...
@router.get(
    "/profiles/{user_id}/contexts/{context_id}/resolved", response_model=ResolvedProfileResponse
)
def get_resolved_context_profile(
    user_id: UUID,
    context_id: UUID,
    include_deprecated: bool = Query(
        default=False, description="Include deprecated names in results"
    ),
...
@router.get("/profiles/{user_id}/resolved", response_model=ResolvedProfileResponse)
def get_resolved_base_profile(
    ...
    try:
        resolved = service.resolve_base_profile(
            user_id=user_id, language=language, include_deprecated_names=include_deprecated
        )
...
@router.patch("/profiles/{user_id}/contexts/{context_id}", response_model=ContextProfileResponse)
def update_context_profile(
    ...
    try:
        ...
        updated = service.update_context_profile(
            context_id=context_id,
            context_name=(update_data.context_name if "context_name" in provided_fields else UNSET),
...
@router.delete("/profiles/{user_id}/contexts/{context_id}", status_code=status.HTTP_204_NO_CONTENT)
