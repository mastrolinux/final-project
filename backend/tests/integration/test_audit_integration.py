...
class TestAuthAuditIntegration:
    ...
    def test_registration_creates_audit_log(self, mock_email, client, db_session):
        ...
        logs = db_session.query(AuditLog).filter(AuditLog.event_type == "auth.register").all()
        assert len(logs) >= 1
    ...
    @patch("src.services.auth_service.send_verification_email")
    def test_login_success_creates_audit_log(self, mock_email, client, db_session):
        ...
        response = client.post(
            "/api/v1/auth/login",
            json={"email": "audit.login@example.com", "password": "SecurePass123!"},
        )
        assert response.status_code == 200

        logs = db_session.query(AuditLog).filter(AuditLog.event_type == "auth.login.success").all()
        assert len(logs) >= 1
        assert logs[0].operation.value == "login"

    @patch("src.services.auth_service.send_verification_email")
    def test_login_failure_creates_audit_log(self, mock_email, client, db_session):
        ...
        response = client.post(
            "/api/v1/auth/login",
            json={"email": "audit.fail@example.com", "password": "WrongPassword123!"},
        )
        assert response.status_code == 401

        logs = db_session.query(AuditLog).filter(AuditLog.event_type == "auth.login.failure").all()
        assert len(logs) >= 1
...
class TestProfileAuditIntegration:
    ...
    def test_create_profile_creates_audit_log(self, client, db_session):
        ...
        logs = db_session.query(AuditLog).filter(AuditLog.event_type == "profile.create").all()
        assert len(logs) >= 1
    ...
    def test_update_profile_creates_audit_log(self, client, db_session):
        ...
        db_session.commit()
        token = create_access_token(user_id=user_id, email=auth_user.email, account_type="verified")
        ...
        logs = db_session.query(AuditLog).filter(AuditLog.event_type == "profile.update").all()
        assert len(logs) >= 1
    ...
    def test_delete_profile_creates_audit_log(self, client, db_session):
        ...
        create_resp = client.post(
            "/api/v1/profiles",
            json={"account_type": "unverified", "primary_email": "audit.delete@example.com"},
        )
        user_id = create_resp.json()["user_id"]
        ...
        logs = db_session.query(AuditLog).filter(AuditLog.event_type == "profile.delete").all()
        assert len(logs) >= 1
...
class TestAuditHashChainIntegrity:
    ...
    @patch("src.services.auth_service.send_verification_email")
    def test_multiple_operations_maintain_hash_chain(self, mock_email, client, db_session):
        ...
        first_entry = db_session.query(AuditLog).order_by(AuditLog.created_at.asc()).first()
