...
def test_create_and_query_profile(db_session: Session):
    ...
    queried = (
        ...
        .filter(BaseProfile.primary_email == "test@example.com")
        .first()
    )
...
def test_create_profile_with_names(db_session: Session):
    ...
    queried = db_session.query(BaseProfile).filter(BaseProfile.user_id == profile.user_id).first()
...
def test_query_jsonb_names(db_session: Session):
    ...
    profile = BaseProfile(primary_email="jsonb@example.com", preferred_language="zh")
    db_session.add(profile)
    db_session.flush()
    ...
    queried_name = (
        db_session.query(IdentityName).filter(IdentityName.identity_id == profile.user_id).first()
    )
...
def test_soft_delete_filtering(db_session: Session):
    ...
    profile2.deleted_at = datetime.now(UTC)
    db_session.commit()

    active_profiles = db_session.query(BaseProfile).filter(BaseProfile.deleted_at.is_(None)).all()
...
def test_account_type_filtering(db_session: Session):
    ...
    verified = BaseProfile(account_type=AccountType.verified, primary_email="verified@example.com")
    unverified = BaseProfile(
        account_type=AccountType.unverified, primary_email="unverified@example.com"
    )
    pseudonymous = BaseProfile(
        account_type=AccountType.pseudonymous, primary_email="pseudonymous@example.com"
    )
    ...
    verified_profiles = (
        db_session.query(BaseProfile).filter(BaseProfile.account_type == AccountType.verified).all()
    )
...
def test_deprecated_names_filtering(db_session: Session):
    ...
    active_names = (
        ...
        .filter(IdentityName.identity_id == profile.user_id, IdentityName.is_deprecated == False)
        .all()
    )
...
def test_transaction_rollback(db_session: Session):
    ...
    queried = db_session.query(BaseProfile).filter(BaseProfile.user_id == user_id).first()
...
def test_bulk_insert_profiles(db_session: Session):
    ...
    profiles = [BaseProfile(primary_email=f"user{i}@example.com") for i in range(10)]
