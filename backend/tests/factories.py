...
class ChineseNameFactory(IdentityNameFactory):
    ...
    name_type = NameType.full_name
    name_value = factory.LazyFunction(lambda: {"zh": fake_zh.name(), "en": fake_zh.name()})
...
class MultilingualNameFactory(IdentityNameFactory):
    ...
    name_value = factory.LazyFunction(
        lambda: {"en": fake.first_name(), "zh": fake_zh.first_name(), "es": fake_es.first_name()}
    )
...
class MononymFactory(IdentityNameFactory):
    ...
    name_type = NameType.full_name
    name_value = factory.LazyFunction(
        lambda: {"id": fake_id.first_name(), "en": fake_id.first_name()}
    )
...
def create_profile_with_names(
    session: Any, account_type: AccountType = AccountType.verified, num_names: int = 2
    ...
    for i in range(num_names):
        ...
        name = name_factory.build(identity_id=profile.user_id, is_primary=(i == 0))
        session.add(name)
        names.append(name)
...
def create_diverse_profiles(session: Any) -> list[BaseProfile]:
    ...
    western_name = WesternNameFactory.build(
        identity_id=western_profile.user_id, name_value={"en": "Sarah Chen"}
    )
    session.add(western_name)
    profiles.append(western_profile)

    chinese_profile = UnverifiedProfileFactory.build(
        primary_email="li.ming@example.com", preferred_language="zh"
    )
    session.add(chinese_profile)
    session.flush()

    chinese_name = ChineseNameFactory.build(identity_id=chinese_profile.user_id)
    session.add(chinese_name)
    profiles.append(chinese_profile)

    mononym_profile = VerifiedProfileFactory.build(
        legal_name="Sukarno", primary_email="sukarno@example.id", preferred_language="id"
    )
    session.add(mononym_profile)
    session.flush()

    mononym_name = MononymFactory.build(identity_id=mononym_profile.user_id)
    session.add(mononym_name)
    profiles.append(mononym_profile)

    pseudonymous_profile = PseudonymousProfileFactory.build(
        primary_email="anonymous@protonmail.com", preferred_language="en"
    )
    session.add(pseudonymous_profile)
    session.flush()

    pseudo_name = PreferredNameFactory.build(
        identity_id=pseudonymous_profile.user_id, name_value={"en": "Alex"}
    )
