...
def _make_auth_user(user_id, deleted_at=None, email="test@example.com"):
    ...
    auth.is_admin = False
    auth.provider = None
    auth.created_at = datetime.now(UTC)
...
class TestSoftDeleteAccount:
    ...
    def test_soft_delete_sets_deleted_at_on_all_tables(
        self, privacy_service, mock_profile_repo, mock_auth_repo, mock_context_repo, mock_oauth_repo
    ):
    ...
    def test_soft_delete_raises_on_missing_profile(self, privacy_service, mock_profile_repo):
    ...
    def test_soft_delete_raises_on_already_deleted(
        ...
        user_id = uuid4()
        mock_profile_repo.get_profile_by_id.return_value = _make_profile(user_id)
        auth = _make_auth_user(user_id, deleted_at=datetime.now(UTC))
        mock_auth_repo.get_by_user_id.return_value = auth
    ...
    def test_oauth_tokens_revoked_on_soft_delete(
        self, privacy_service, mock_profile_repo, mock_auth_repo, mock_oauth_repo
    ):
    ...
    def test_audit_log_created_for_deletion(
        self, privacy_service, mock_profile_repo, mock_auth_repo, mock_audit_service
    ):
...
class TestDeletionStatus:
    ...
    def test_deletion_status_active(self, privacy_service, mock_auth_repo):
    ...
    def test_deletion_status_scheduled(self, privacy_service, mock_auth_repo):
    ...
    def test_deletion_status_purged(self, privacy_service, mock_auth_repo):
...
class TestPurgeExpiredAccounts:
    ...
    def test_permanent_purge_cascades(
        self, privacy_service, mock_auth_repo, mock_profile_repo, mock_audit_service
    ):
    ...
    def test_purge_returns_zero_when_no_expired(self, privacy_service, mock_auth_repo):
...
class TestLoginSoftDeleted:
    ...
    def test_soft_deleted_user_login_returns_account_deleted(self, auth_service, mock_auth_repo):
        ...
        success, error, data = auth_service.login(email="test@example.com", password="AnyPass123!")
    ...
    def test_expired_soft_deleted_user_login_returns_invalid(self, auth_service, mock_auth_repo):
...
class TestRequestAccountRestoration:
    ...
    def test_restore_account_nonexistent_email_returns_success(self, auth_service, mock_auth_repo):
    ...
    def test_restore_account_active_account_returns_success(self, auth_service, mock_auth_repo):
        """Active (not deleted) account returns success without action."""
        active_user = _make_auth_user("00000000-0000-0000-0000-000000000099", deleted_at=None)
...
class TestConfirmAccountRestoration:
    ...
    def test_confirm_restoration_invalid_token_returns_error(self, auth_service, mock_auth_repo):
    ...
    def test_confirm_restoration_expired_token_returns_error(self, auth_service, mock_auth_repo):
    ...
    def test_confirm_restoration_weak_password_returns_error(self, auth_service, mock_auth_repo):
    ...
    def test_audit_log_created_for_restoration(
        self, auth_service, mock_auth_repo, mock_profile_repo, mock_audit_service
    ):
