...
from src.models.profile import BaseProfile, IdentityName, NameType, VisibilityLevel
from src.models.verification import DocumentType, VerificationDocument, VerificationStatus
...
class TestContextCreation:
    ...
    def test_create_context_profile_success(
        self, context_service: ContextService, sample_verified_profile: BaseProfile
    ):
    ...
    def test_create_context_for_nonexistent_user(self, context_service: ContextService):
        ...
        with pytest.raises(ContextServiceError) as exc_info:
            context_service.create_context_profile(
                user_id=fake_user_id, context_type=ContextType.professional, context_name="Work"
            )

        assert "not found" in str(exc_info.value).lower()

    def test_pseudonymous_cannot_create_legal_context(
        self, context_service: ContextService, sample_pseudonymous_profile: BaseProfile
    ):
    ...
    def test_unverified_creates_legal_context_pending(
        self, context_service: ContextService, sample_unverified_profile: BaseProfile
    ):
...
class TestInheritanceEngine:
    ...
    def test_resolve_context_with_full_overrides(
        ...
        resolved = context_service.resolve_context_profile(
            user_id=sample_verified_profile.user_id, context_id=context.id
        )
    ...
    def test_resolve_context_does_not_inherit_base_avatar(
        self, db_session, context_service: ContextService, sample_verified_profile: BaseProfile
    ):
        """
    ...
    def test_resolve_base_profile_without_context(
        ...
        resolved = context_service.resolve_base_profile(user_id=sample_verified_profile.user_id)
    ...
    def test_resolve_context_not_belonging_to_user_fails(
        ...
        with pytest.raises(ContextServiceError) as exc_info:
            context_service.resolve_context_profile(
                user_id=sample_unverified_profile.user_id, context_id=context.id
            )
...
class TestContextManagement:
    ...
    def test_update_context_profile(
        ...
        updated = context_service.update_context_profile(
            context_id=context.id, bio="New bio", email_override="new@work.com"
        )
...
class TestMultilingualNameResolution:
    ...
    def test_resolve_exact_language_match(
        ...
        resolved = context_service.resolve_context_profile(
            user_id=sample_verified_profile.user_id, context_id=context.id, language="zh"
        )
        ...
        resolved_value = context_service._resolve_multilingual_name(
            name, requested_language="zh", preferred_language="en"
        )
    ...
    def test_fallback_to_preferred_language(
        ...
        name = resolved.identity_names[0]
        resolved_value = context_service._resolve_multilingual_name(
            name, requested_language="fr", preferred_language="zh"
        )
        assert resolved_value == "李明"
    ...
    def test_fallback_to_first_available(
        ...
        resolved = context_service.resolve_context_profile(
            user_id=sample_verified_profile.user_id, context_id=context.id, language="de"
        )

        name = resolved.identity_names[0]
        resolved_value = context_service._resolve_multilingual_name(
            name, requested_language="de", preferred_language="fr"
        )
    ...
    def test_empty_name_value_returns_empty_string(
        ...
        resolved_value = context_service._resolve_multilingual_name(
            empty_name, requested_language="en", preferred_language="en"
        )
...
class TestTemporalValidity:
    ...
    def test_resolve_expired_context_raises_410(
        ...
        with pytest.raises(ContextServiceError) as exc_info:
            context_service.resolve_context_profile(
                user_id=sample_verified_profile.user_id, context_id=expired_context.id
            )
...
class TestExpiryEnrichment:
    ...
    def test_get_context_returns_expired_when_document_expired(
        ...
        doc = self._make_verified_document(db_session, sample_verified_profile.user_id, yesterday)
    ...
    def test_get_context_returns_verified_when_document_not_expired(
        ...
        doc = self._make_verified_document(db_session, sample_verified_profile.user_id, future)
